...
현재 코드는 아직 Spring Batch chunk `Step`으로 전체 job을 재작성하지 않는다. 다만 Spring Batch 6.x JDBC `JobRepository`는 도입되어 있으며, `StockBatchJobRunner`가 custom job 실행을 `JobExecution`/`StepExecution` metadata로 기록한다. 재시작 가능한 대량 chunk 처리, 원격 step/chunk가 필요해지는 시점에는 도메인별 job을 Spring Batch `Job`/`Step`으로 점진 전환한다.

현재 custom scheduler job은 metadata를 재시작 가능한 chunk 처리 상태가 아니라 고빈도 실행 이력 ledger로 사용한다. 그래서 `businessDate`, `jobMode`, `runId`를 identifying parameter로 기록한다. 같은 업무일과 같은 mode라도 스케줄이 반복 실행되면 별도 `JobInstance`가 남아야 하기 때문이다. 이 예외는 현재 command-style job에만 적용하며, 재시작 가능한 chunk job을 추가할 때는 stable identifying parameter와 restart point를 별도 설계한다.
...
- `businessDate`
- `jobMode`
- `runId`: 현재 custom scheduler 실행 이력 ledger job에만 적용하는 예외
...
- 현재 custom scheduler job의 `runId`는 같은 업무일/모드 반복 실행을 Spring Batch metadata에 별도 `JobInstance`로 남기기 위한 예외다. 이 값은 재시작 가능한 chunk job의 표준 identifying parameter로 사용하지 않는다.
