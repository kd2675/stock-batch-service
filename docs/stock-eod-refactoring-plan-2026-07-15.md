...
서버 종료를 확인한 유지보수 창에서 다음 11개 ALTER만 순서대로 적용한다. `*_alter.sql` 전체를 다시 일괄 실행하지 않는다. 과거 주문·체결 인덱스 ALTER까지 다시 실행하면 현재 추정 주문 151만/체결 103만 이상 테이블의 DDL 여부 확인과 인덱스 작업 부하를 불필요하게 재발생시킬 수 있다. 12번은 스키마 ALTER가 아니라 별도 일회성 데이터 보정이며, 1~11번 스키마가 준비된 뒤 사전 대사를 통과했을 때만 실행한다.
