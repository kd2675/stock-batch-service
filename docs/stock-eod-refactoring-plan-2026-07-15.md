...
- 체결의 매수·매도 계좌 잠금도 계좌별 두 번의 원격 조회가 아니라 두 PK 값을 가진 2행 derived table과 계좌 PK를 조인하고 PK 오름차순을 사용한 한 번의 `FOR UPDATE`로 통합한다. 잠금 뒤 계좌·주문 CASE UPDATE도 MySQL에서는 동일한 2행 derived PK join과 `FORCE INDEX(PRIMARY)`를 사용해 `id IN (...)` range 계획 및 인접 INSERT gap-lock 회귀 가능성을 남기지 않는다. 잠금 순서는 유지하면서 체결 한 건당 DB 왕복을 1회 줄이고, 두 계좌 중 하나라도 사라진 비정상 상태는 원장 변경 전에 실패 처리
...
- 계좌 활성 상태와 정산 대상 여부
- 장마감 전 가용 현금 — 주문 반환 대사용 내부 감사값
- 장마감 전 매수 예약금 — 주문 반환 대사용 내부 감사값
- 유상증자 청약 대기자산
- 미체결 취소 후 최종 가용 현금 — 포트폴리오 정산·화면의 현금 기준
- 외부 순입금 watermark
- 장마감 평가가격 기준 보유 평가액
...
포트폴리오 정산은 주문 취소와 예약금 반환이 끝난 장마감 최종 상태를 사용합니다. 취소 전 현금과 매수 예약금은 `post_cancel_cash = pre_cancel_cash + pre_cancel_order_reserved_cash` 대사를 위해 close snapshot에 한 번만 남기고, 포트폴리오 현금이나 사용자 일별 자산 구성에는 다시 노출하지 않습니다.
...
여기서 `총 소유수량 = 가용 보유수량 + 예약 매도수량`입니다. 청약 대기자산은 신주 상장 전까지 사용할 수 없는 자산이므로 가용 현금에 합치지 않습니다. 장마감 전 매수 예약금과 취소 후 현금을 동시에 더하거나, 총 소유수량에 예약 매도수량을 다시 더하면 이중 계산이 됩니다. 이 변경은 close snapshot과 정산 reader만 사용하며 정규장 `stock_order`·`stock_execution` 쓰기, 잠금, 인덱스에는 영향을 주지 않습니다.

신규 정산은 `portfolio-v4-explicit-subscription-asset` 계산 버전으로 `portfolio_snapshot.cash_balance = post_cancel_cash`와 `pending_subscription_asset = subscription_reserved_cash`를 각각 저장합니다. 기존 v2/v3 행은 `stock_close_account_snapshot`과 대사가 모두 일치할 때만 취소 후 현금·청약 대기자산·v4 입력 hash·계산 버전을 함께 갱신합니다. 불변 입력 도입 전 행은 예외 분기나 잔차 추정을 런타임에 남기지 않고, 각 스냅샷 `created_at`에서 유효했던 `subscribed_at <= created_at < paid_at` 청약 원장을 합산해 청약 대기자산을 복원한 뒤 `cash_balance = total_asset - market_value - pending_subscription_asset`으로 보정합니다. 관리자 일별 이력은 계산 버전 분기나 잔차식을 사용하지 않고 명시 컬럼만 집계합니다. 보정은 저빈도 정산·기업행사 권리 테이블만 읽으며 `stock_order`·`stock_execution`을 읽거나 변경하지 않습니다.

수익률 계약은 `portfolio-v5-net-contribution-return`부터 별도로 강화합니다. `portfolio_snapshot`에 `net_contribution`, `total_profit`, `return_rate_status`를 저장하고, 손익은 `total_asset - net_contribution`으로 항상 계산합니다. 순입금이 양수인 행만 `DEFINED` 수익률을 가지며 0원·음수 순입금은 각각 `UNDEFINED_ZERO_CONTRIBUTION`, `UNDEFINED_NEGATIVE_CONTRIBUTION`과 `NULL` 수익률로 저장합니다. 장마감 입력과 정확히 연결되는 기존 행만 v5로 보정하고, 증명할 수 없는 구형 행은 `LEGACY_UNVERIFIED`로 남겨 대표 성과와 랭킹에서 제외합니다. 이 보정도 장마감 불변 스냅샷만 사용하므로 정규장 주문·체결 원장과 잠금에 영향을 주지 않습니다.

`portfolio_snapshot`에는 다음을 추가합니다.
...
계좌 스냅샷은 기본 500계좌씩 생성하므로 watermark 범위를 계좌 청크 수만큼 PK로 다시 훑지 않게 `stock_account_cash_flow(account_id, id)`를 사용합니다. 청약 대기자산도 `stock_corporate_action_entitlement(account_id, status)`로 해당 계좌·`SUBSCRIBED` 권리만 읽고, 반환 후 현금 대사는 `stock_close_account_snapshot(close_cycle_id, reconciliation_status, account_id)` keyset으로 진행합니다. 이 세 인덱스의 쓰기 비용은 현금흐름·권리·EOD 스냅샷 생성에만 발생하고 주문·체결 원장의 정규장 쓰기 증폭은 0입니다.
...
체결의 두 계좌는 별도 PK 조회 두 번으로 잠그지 않고 두 PK 값을 가진 2행 derived table을 계좌 PK에 조인한 `ORDER BY stock_account.id` 단일 `FOR UPDATE`로 잠급니다. MySQL의 계좌·주문 CASE UPDATE도 같은 derived PK join을 사용해 두 행을 `eq_ref`로 변경하며, 잠금만 exact PK로 바꾼 뒤 실제 UPDATE가 다시 `id IN (...)` range scan을 수행하는 빈틈을 남기지 않습니다. 이에 따라 `fence → 계좌 → 매도 보유 → 정확한 주문 PK` 순서는 그대로 유지하면서 원격 DB 왕복은 체결당 1회 감소합니다. 계좌 두 건이 모두 조회되지 않으면 이후 보유·주문 잠금과 원장 INSERT를 실행하지 않습니다.
...
- 청약 대기자산은 `(account_id, status)`, 반환 대사 paging은 `(close_cycle_id, reconciliation_status, account_id)`를 사용하며, 신규 인덱스는 startup schema readiness에서 필수로 검증
...
서버 종료를 확인한 유지보수 창에서 다음 파일만 순서대로 적용한다. `*_alter.sql` 전체를 다시 일괄 실행하지 않는다. 과거 주문·체결 인덱스 ALTER까지 다시 실행하면 현재 추정 주문 151만/체결 103만 이상 테이블의 DDL 여부 확인과 인덱스 작업 부하를 불필요하게 재발생시킬 수 있다. 12~13번은 포트폴리오 스냅샷 스키마와 데이터를 단계적으로 보정하며, 1~11번 스키마가 준비된 뒤 사전 대사를 통과했을 때만 실행한다.

아래 1~11번은 `STOCK_SERVICE` business schema용 ALTER다. metadata 보존을 운영 승인할 때는 백엔드·배치 종료와 실행 중 native Job 0건을 별도로 확인한 뒤 `stock-batch-service/src/main/resources/db/schema/batch-metadata-retention-mysql.sql`을 `STOCK_BATCH_METADATA`에 적용한다. 이 파일은 compact archive 테이블과 `BATCH_JOB_EXECUTION` 후보 인덱스만 만들며 business ALTER 11개에 포함하지 않는다. 적용 전후 `stock_order`·`stock_execution` 정의는 바뀌지 않아야 하고, 적용 직후에도 `STOCK_BATCH_METADATA_RETENTION_ENABLED=false`, `PURGE_ENABLED=false`를 유지한다.
...
12번이 가용 현금·청약 대기자산 구성을 확정한 뒤 13번이 순입금·손익·수익률 상태를 자기완결적으로 저장합니다. 13번은 close cycle과 계좌 스냅샷이 정확히 연결되고 대사가 완료된 행만 v5로 보정하며, 그 외 과거 행은 `LEGACY_UNVERIFIED`로 보존합니다. 재적용은 구조 검증과 같은 값의 멱등 보정만 수행하고, 두 파일 모두 정규장 주문·체결 원장을 읽거나 변경하지 않습니다.
...
12번은 `portfolio_snapshot`의 현금을 취소 후 가용 현금으로 단일화하고 청약 대기자산을 독립 컬럼으로 영속화하는 일회성 데이터 보정입니다. v2/v3 행은 immutable 계좌 스냅샷 대사가 모두 일치할 때만 현금·청약 대기자산·계산 버전·SHA-256 입력 hash를 v4로 함께 갱신합니다. immutable 입력 도입 전 행은 당시 청약 원장의 구독·납입 시각 범위로 대기자산을 복원하고 나머지 잔차를 가용 현금으로 보정하며 `WARNING`으로 출처 한계를 남깁니다. 최종 DB CHECK는 `총자산 = 가용 현금 + 청약 대기자산 + 평가액`을 강제합니다. 임시 target PK로만 결과를 갱신하고 `stock_order`·`stock_execution`을 읽거나 변경하지 않습니다. 검증 실패 시 CHECK guard가 COMMIT 전에 데이터 변경을 중단하고, 재적용 시 동일 v4 값으로 수렴합니다. 운영 적용은 백엔드·배치 서버 종료 유지보수 창에서 수행합니다.
...
2026-07-24 체결 리팩토링 재검토에서 업무 MySQL 8.0.32를 읽기 전용으로 확인한 시점의 `stock_order`는 2,735,785행, `stock_execution`은 2,032,244행이었습니다. 격리수준은 `REPEATABLE-READ`였고 미체결 주문과 OPEN 시장은 모두 0건, 활성 종목은 3건이어서 현재 스냅샷으로 장중 후보 SQL p95/p99를 측정할 수는 없었습니다. 2행 derived PK 잠금과 MySQL 전용 계좌·주문 CASE UPDATE는 실제 optimizer에서 모두 대상 테이블 `eq_ref`·`PRIMARY` 계획을 확인했습니다. 반면 후보 CTE는 현재 통계에서 `idx_stock_order_market_status_symbol` range scan과 filesort를 선택했습니다. 미체결 0건의 낮은 비용 계획은 장중 대형 호가창의 성능 증거가 아니므로, 동일 데이터 canary에서 후보 조회 timer·rows examined·체결 반응 p95/p99를 확인하기 전에는 후보 CTE 성능을 승인하지 않습니다.

같은 점검에서 프로젝트 Redis endpoint는 응답했고 ready-symbol Set/List는 모두 존재하지 않는 빈 상태였으며 lease/cooldown도 없었습니다. 실제 소스에서 추출한 reconcile Lua는 존재하지 않는 격리 키와 빈 후보로 읽기 전용 실행해 정상 반환을 확인했습니다. 이 결과는 Lua 구문과 빈 경로만 검증하며, 장중 queue backlog·다중 인스턴스 lease·네트워크 지연 부하를 통과했다는 의미는 아닙니다.
