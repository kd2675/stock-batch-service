...
- `AutoMarketScheduler`: 자동 참여자 주문 생성, 자동장 미체결 주문 만료, LP·인수기관·기관투자자 주문 job을 실행
...
`mysqlTest`는 MySQL 8 Testcontainers에서 실제 운영 조인 형태의 `FOR SHARE OF f` session fence drain과 전역 제어행 비잠금, 취소·정정의 소유 주문 조회가 fence만 잠그고 주문·계좌·시장설정·시계 행을 미리 잠그지 않는지, derived exact-PK 조인으로 주문·계좌 두 행을 한 번에 잠그면서 인접 INSERT에는 gap lock을 만들지 않는지, 기존 `(market_type, status, symbol)` 보조 인덱스의 확장 PK를 사용한 열린 주문 `id` keyset·filesort 비회귀, signal 후보 조회 후 exact-PK `SKIP LOCKED`가 잠긴 선두를 건너뛰는지, canonical DDL과 운영 EOD ALTER 묶음의 재실행 멱등성, 구형 스키마의 누락 컬럼·제약·인덱스 생성 및 기존 signal/스냅샷/체결 손익 요약 backfill, 적용 전후 hot-ledger DDL 비변경을 검증하는 격리 테스트입니다. 일반 `test`와 `check`에는 Docker 기동 비용을 넣지 않습니다. 명시적으로 실행한 `mysqlTest`는 Docker를 사용할 수 없으면 컨테이너 초기화에서 실패하여 MySQL 검증이 실행되지 않은 상태를 성공으로 숨기지 않습니다. 외부 또는 로컬 업무 MySQL URL을 대신 주입하는 경로는 제공하지 않습니다. 승인 증거에는 결과 XML의 `tests/skipped/failures/errors`를 함께 기록하며, 1건이라도 skipped이면 실제 MySQL 동시성·주문/체결 부하 검증을 통과한 것으로 판정하지 않습니다.
...
- 축소시장 역할 재구성 ALTER의 기준 파일과 적용 순서는 stock-back README를 따릅니다. batch에 둔 역할 ALTER 사본은 배포 전에 canonical 사본과 byte 단위로 같아야 합니다. 과거 역할 통합, 기존 자동유동성 계좌의 전량 이전·레거시 테이블 삭제와 완료된 과거 인수 이력 복원은 운영 원장 소유자인 stock-back의 `stock_obsolete_participant_role_cleanup_alter.sql`, `stock_legacy_liquidity_retirement_alter.sql`, `stock_legacy_underwriting_history_backfill.sql`만 순서대로 적용하며 batch 사본을 두지 않습니다. 세 파일 모두 서비스와 시뮬레이션 시계가 정지된 유지보수 창에서 실행합니다. 역할 스키마 readiness와 레거시 테이블 부재 검사가 모두 통과하기 전에는 기관 LIVE, LP LIVE, 인수기관 공급 job을 켜지 않습니다.
...
- `stock.batch.auto-market-order-expiry.market-maker-replacement-limit-per-side`: V2 시장조성 호가의 TTL 만료와 가격 이탈 교체를 합쳐 한 종목·방향·실행에서 취소할 상한입니다. 기본값 10, 허용 범위 1~100입니다.
- `stock.batch.auto-market-order-expiry.market-maker-reprice-threshold-ticks`: 동일 방향 최우선 호가에서 이 틱 수보다 멀어진 V2 시장조성 호가만 TTL 전에 교체합니다. 기본값 2, 허용 범위 1~100입니다.
- `stock.batch.auto-market-order-expiry.market-maker-minimum-quote-lifetime-seconds`: 가격이 이탈해도 기존 V2 시장조성 호가를 유지할 최소 시뮬레이션 시간입니다. 기본값 30초, 허용 범위 1~86,400초입니다.
- 만료 후보는 `idx_stock_order_market_status_symbol`로 현재 미체결 집합을 먼저 줄인 뒤, 신규 주문은 주문에 고정된 `expires_at`, 구형 NULL 주문은 주문에 고정된 프로필 또는 현재 참여자 프로필의 TTL을 SQL 후보 단계에서 정확히 적용합니다. 짧은 TTL의 넓은 기준으로 먼저 LIMIT한 뒤 애플리케이션에서 거르는 방식은 장기 TTL 주문이 뒤의 실제 만료 주문을 막을 수 있으므로 사용하지 않습니다. 후보 발견은 체결과 공유하는 Redis 종목 락 밖에서 계정·프로필까지 한 SQL로 읽고, 락 안에서는 세션 fence와 후보 PK `FOR UPDATE SKIP LOCKED` 재검증·취소·예약 반환만 수행합니다.
- V2 시장조성 재호가 후보는 한 만료 run에서 소형 참여자·계좌 테이블의 V2 시장조성 계정을 한 번만 제한 조회한 뒤, 종목별 후보는 Redis 종목 락 밖에서 기존 `idx_stock_order_account_status_created`로만 찾습니다. 실제 후보가 없는 종목은 락을 시도하지 않으며, 모든 주문 쓰기에 비용을 추가하던 `idx_stock_order_auto_reprice`는 canonical schema에서 제외하고 `stock_auto_market_reprice_index_alter.sql` 재적용 시 기존 인덱스도 제거합니다.
- 자동 참여자 V2 운영 ALTER는 서버를 종료한 유지보수 창에서 아래 순서로만 적용합니다. `stock_auto_participant_behavior_state_alter.sql`이 `stock_order.funding_budget_type`을, `stock_auto_order_policy_snapshot_alter.sql`이 `expires_at`·프로필·모델 스냅샷 컬럼을 만든 뒤 `stock_auto_market_reprice_index_alter.sql`이 세 CHECK를 한 번의 hot-ledger ALTER로 추가합니다. 마지막 파일을 먼저 실행하거나 `*_alter.sql`을 파일명순으로 일괄 실행하지 않습니다.
...
- 기관 LIVE는 미체결을 포함한 예상 포지션, 최근 완료 20거래일 ADV 기반 종목 참여율, 일일 AUM 총매매, 결정당·단일 주문 한도를 모두 적용합니다. 일일·결정 한도는 체결 할당량이 아니라 상한이며, 종목별 자식 주문은 ADV 1.5% 이하이고 공격 주문은 외부 5호가 잔량 10% 이하입니다. 한 주문 의도가 3회 실패하면 포트폴리오와 정책 버전을 자동 `SUSPENDED`로 고정하고, 나머지 대기 의도를 거절한 뒤 전용 계좌의 기존 미체결 주문도 같은 트랜잭션에서 취소해 현금·주식 예약을 반환합니다.
...
- LP 공급의 활성 종목 조회는 자동 참여자용 최신 보고서 점수·분포 편향 상관 서브쿼리를 사용하지 않습니다. LP 계약·일일 상태·호가 깊이·가격·재고에 필요한 bounded 쿼리만 사용합니다.
- 레거시 자동 유동성 job·설정·런타임 제어는 제거했습니다. 과거 자동 유동성 주문 출처는 역할 정리 migration에서 `LIQUIDITY_PROVIDER`로 통합하고 종료된 계좌·체결은 감사 이력으로 보존합니다.
...
- `stock_portfolio_snapshot_post_close_cash_data_fix.sql`은 `portfolio_snapshot.pending_subscription_asset`을 명시적으로 추가하고 과거 스냅샷의 자산 구성을 정규화합니다. v2/v3 행은 immutable 계좌 스냅샷에서 취소 후 가용 현금·청약 대기자산·v4 입력 hash를 정확히 복원하고, immutable 입력 이전 행은 각 스냅샷 시각의 `subscribed_at`/`paid_at` 원장 범위로 청약 대기자산을 복원한 뒤 나머지 매수예약 잔차만 가용 현금에 합칩니다. 최종 불변식은 `총자산 = 가용 현금 + 청약 대기자산 + 평가액`이며 관리자 이력도 이 명시 컬럼만 집계합니다. 보정은 저빈도 `portfolio_snapshot`·close snapshot·기업행사 권리 원장만 사용하고 `stock_order`·`stock_execution`은 읽거나 변경하지 않습니다.
- 이어서 `stock_portfolio_snapshot_return_contract_alter.sql`을 적용합니다. 이 파일은 `net_contribution`, `total_profit`, `return_rate_status`와 nullable `DECIMAL(19,8)` 수익률 계약을 추가하고, immutable close account snapshot과 정확히 연결되는 행만 `portfolio-v5-net-contribution-return`으로 보정합니다. 순입금이 0원·음수인 계좌는 수익률을 0%로 만들지 않고 상태와 `NULL`로 구분하며, 증명할 수 없는 구형 행은 `LEGACY_UNVERIFIED`로 남깁니다. 두 서비스의 SQL은 byte 동일하고 정규장 주문·체결 원장을 읽거나 변경하지 않습니다.
...
- 액면분할 확정 트랜잭션은 보유·발행량과 가격뿐 아니라 자동시장 기준거래량, 기관 종목 mandate, LP 기준거래량·목표재고·주문상한도 같은 배수로 보정합니다. 그렇지 않으면 분할로 정상 증가한 LP 재고를 과잉 재고로 오판할 수 있습니다. 무상증자·주식배당의 계좌별 정수 배정 후 남는 단주는 유일한 `SYSTEM_CUSTODY` 계정에 비거래 수량으로 입고하고 `stock_security_allocation_ledger`에 남겨, 공시 발행량과 실제 보유량이 보존되도록 합니다.
...
- 자동 주문·자동 주문 만료·발행 인수기관 공급·fallback 체결 scheduler도 DB 시장 gate가 닫혀 있으면 주문 후보·계좌·보유 테이블을 읽기 전에 반환합니다. 이 admission gate는 `stock_order`와 `stock_execution`을 조회하지 않습니다.
...
- 보고서 집계 서비스 자체도 full-market cycle의 phase가 정확히 `OVERNIGHT_CASH_APPLIED`일 때만 실행합니다. coordinator 외의 수동·오래된 호출이 freeze/정산과 겹쳐 `stock_execution` 범위 집계를 시작하거나, 이미 완료된 보고서를 다시 삭제·작성하는 경로를 fail-closed로 막습니다.
- coordinator가 호출하는 각 launcher도 이미 읽은 full-market cycle 행에서 정확한 선행 phase를 다시 확인합니다. 기업행사 현금은 `PORTFOLIO_SETTLED`, 자동 현금은 `CORPORATE_CASH_APPLIED`, 보고서는 `OVERNIGHT_CASH_APPLIED`, 보유 정리·PRE_OPEN 변환은 `REPORTS_AGGREGATED`, 시세 준비는 `PREOPEN_SECURITY_TRANSFORMS_APPLIED`, regime·프로필 큐는 `MARKET_DATA_PREPARED`, readiness는 `AUTO_MARKET_PREPARED`에서만 시작합니다. 기업행사 현금을 T+1 정기입금보다 먼저 적용하여 T 거래일의 유상증자 납입 가능 현금에 다음 날 새 자금이 섞이지 않게 합니다. 이 검증은 제어행 1회 조회뿐이며 `stock_order`·`stock_execution`을 읽기 전에 실패하므로 직접 호출이나 오래된 신호도 무거운 야간 작업을 정규장/잘못된 단계에 겹치게 하지 못합니다.
...
- 05:30 개장 readiness는 포트폴리오·거래일·시장·종목 fence·가격·발행수량 보존·기업행사·regime·프로필 큐·EOD contract compatibility의 고정 11개 결과를 `stock_post_close_readiness_check`에 cycle당 최대 11행으로 저장합니다. 발행수량 보존 검사는 모든 장전 기업행사가 끝난 뒤 enabled 종목만 대상으로 `stock_holding(symbol, account_id)` 인덱스를 사용해 현재 전체 보유량·예약수량 유효성을 현재 발행량과 한 번 대사합니다. 주문·체결 원장은 읽지 않으며, 주식 소실·중복 또는 초과 예약이 있으면 다음 장 개장을 차단합니다. 서버 재시작·배포로 build SHA나 additive schema revision이 달라져도 동일한 EOD contract이면 기존 cycle을 이어가며, cycle의 최초 build/schema와 phase별 실제 실행 build/schema는 감사 이력으로 각각 보존합니다. 실패 Step도 진단행은 별도 트랜잭션으로 보존하며 관리자 화면은 이 PK 범위만 읽어, 15초 polling이 주문·체결·기업행사 원장을 다시 집계하지 않습니다.
- `STOCK_EOD_CONTRACT_VERSION`은 phase 입력·출력, 불변 스냅샷 의미, 재시작 규칙이 호환되는지를 나타내며 기본값은 `EOD_V1`입니다. 단순 재빌드나 additive DDL에서는 바꾸지 않고, 완료 phase 결과를 새 코드가 안전하게 이어받을 수 없는 breaking EOD 변경에서만 올립니다. readiness는 cycle과 완료 attempt의 이 값이 현재 계약과 모두 정확히 같은 경우에만 통과합니다. build/schema 문자열을 허용 목록으로 관리하지 않으며 `UNDECLARED`·다른 계약은 fail-closed합니다.
...
- 정산 완료 검증은 같은 frozen cohort와 `portfolio_snapshot`을 한 번 조인한 결과에서 입력 hash·산식 버전뿐 아니라 현금·평가액·총자산·외부 순입금·손익·nullable 수익률·수익률 상태·보유수량·예약매도·포지션 수도 다시 계산해 전부 대조합니다. 외부 순입금이 0원 이하이면 손익은 그대로 저장하고 수익률만 산출 불가로 남깁니다. 기존 완료 검증 쿼리에 포함되므로 추가 원장 스캔이나 정규장 이중 쓰기는 없으며, 값이 하나라도 다르면 `PORTFOLIO_SETTLED`로 전진하지 않습니다.
...
- `stock.batch.post-close.report-aggregation.funding-budget-chunk-size`: 야간 보고서 완료 전에 월급·배당 재투자 예산과 주문별 예약 원장을 대사하는 budget cohort 상한입니다. 기본값은 500, 허용 범위는 1~1,000입니다. `budget.id` 키셋으로 작은 전용 원장만 읽으며 `granted=available+reserved+spent`, 주문별 `allocated=remaining+spent+released`, 예산/주문 원장의 예약·사용 합계를 함께 검증합니다. 불일치 시 `REPORTS_AGGREGATED`로 전진하지 않고 같은 Step 체크포인트부터 재시도하며, `stock_order`·`stock_execution` 원장은 스캔하지 않습니다.
...
- `stock.batch.scheduler-pools.auto-market.pool-size`: 자동 참여자 주문 생성, 자동장 주문 만료, 발행 인수기관 주문 공급 전용 scheduler pool 크기. 값은 반드시 1이어야 합니다. 같은 주문/계좌/보유 테이블을 쓰는 주기 작업을 병렬 진입시키지 않습니다.
...
- `stock.batch.execution.buy-candidate-scan-limit`: 주문장 매칭 1회에서 한 SQL로 비교할 매수·매도 상위 후보 수입니다. 기본값은 20, 허용 범위는 1~100입니다. 상위 창이 같은 계좌 주문이나 시장가로 채워진 경우에는 최우선 다른 계좌·지정가 후보를 방향별 소수만 보강해 정상 상대 주문이 뒤에 가려지지 않게 합니다. 후보 탐색에서는 잠금을 잡지 않고, 선택된 주문 PK 2개는 PK 값을 가진 2행 derived table과 조인해 오름차순으로 한 번에 잠근 뒤 재검증합니다. `id IN (...) FOR UPDATE`가 InnoDB에서 인접 auto-increment INSERT까지 next-key lock으로 막는 경로를 사용하지 않습니다.
...
- `stock.batch.execution.ready-symbol-fallback-scan-limit`: Redis 복구 한 회차와 한 worker 호출이 연속 probe할 distinct symbol 상한입니다. 기본값은 8, 허용 범위는 1~100입니다. 상시 worker의 경량 reconcile은 이 크기로 활성 OPEN 종목을 symbol keyset 순환하고, 30초 저주기 fallback은 같은 상한으로 실제 체결 가능 종목을 찾습니다. ready queue에 미체결 종목이 대량 누적돼도 한 호출이 큐 전체를 비우며 후보 SQL을 무제한 실행하지 않고, 남은 symbol은 큐에 그대로 있어 다음 호출이 이어서 처리합니다.
- `stock.batch.execution.ready-symbol-reconciliation-enabled`, `ready-symbol-reconciliation-interval-ms`: Redis enqueue 누락·worker poll 직후 프로세스 종료를 복구하는 경량 reconcile 설정입니다. 기본값은 활성·1000ms이며 허용 간격은 100~30000ms입니다. 다중 worker·다중 인스턴스 중 한 실행만 Redis token lease와 클러스터 공통 cooldown을 얻고, symbol keyset cursor도 lease와 같은 Redis namespace에 공유해 인스턴스마다 첫 페이지를 반복하거나 인스턴스 수만큼 같은 주기의 DB scan을 연속 실행하지 않습니다. reconcile 자체는 `stock_order`·`stock_execution`을 조회하지 않은 채 소형 시장 설정·활성 종목만 PK/index로 순환해 ready-symbol 큐에 넣습니다. 복구 전용 Lua는 먼저 Set/List cardinality를 상수 시간으로 비교합니다. 정상 불변식이면 `SISMEMBER`·`SADD`·`RPUSH`만 사용하고, 실제 cardinality 불일치가 감지된 회차도 List 앞부분 최대 10,000개를 한 번만 읽어 membership map으로 대조합니다. 따라서 후보마다 선형 `LPOS`를 반복하지 않고 복구 최악 비용을 O(N+M)으로 제한합니다. 10,000개 밖의 비정상 List-only 항목은 일시적 중복 probe가 생길 수 있지만 poll 과정에서 제거되며 주문·체결 유실로 바뀌지 않습니다. 이후 worker는 종목별 상위 후보 20건으로 제한된 기존 CTE를 한 번씩 probe하므로, 전 종목 교차쌍 상관 조회를 1초마다 실행하는 구조는 아닙니다.
- `stock.batch.execution.ready-symbol-queue.reconciliation-lease-*`: 경량 reconcile의 Redis lease key와 TTL입니다. 기본 TTL은 3000ms, 허용 범위는 500~60000ms이며 token 일치 Lua로만 해제해 만료 후 다른 인스턴스가 얻은 lease를 삭제하지 않습니다. lease key에서 파생한 cooldown은 획득 시 원자적으로 시작하고 cursor 갱신 성공 시 전체 reconcile 간격으로 연장합니다. 실패 시에도 최소 간격 동안 다른 인스턴스의 연속 재시도를 막되, 영구 키가 아니라 짧은 TTL key라 자동 복구됩니다.
- 체결 성공 쓰기는 매도·매수 계좌를 한 CASE UPDATE, 주문 두 건을 한 CASE UPDATE, 양쪽 체결 원장을 한 multi-row INSERT로 반영합니다. MySQL의 두 CASE UPDATE도 잠금 조회와 같은 2행 derived PK join 및 `FORCE INDEX(PRIMARY)`를 사용해 `WHERE id IN (...)` range 계획으로 되돌아가지 않게 합니다. 후보가 없으면 transaction과 빈 COMMIT을 만들지 않고, 후보 소진·장 종료·건수/시간/전체 상한·stale 상한의 종료 사유로 재큐잉 여부를 결정하므로 후속 `hasExecutablePair` 조회도 실행하지 않습니다.
- 체결 계측은 후보 조회, transaction 전체, callback 진입 전 비용, callback 본문, transaction 완료 구간, 계좌·보유·주문 잠금, 원장 쓰기, after-commit callback, symbol chunk와 Redis reconcile을 분리합니다. transaction 완료 구간에는 JDBC commit 외에 Spring transaction synchronization 종료가 포함될 수 있으므로 이를 DB fsync 단독 시간이라고 부르지 않습니다.
- 여러 체결을 한 transaction으로 묶는 micro-batch는 아직 활성화하지 않습니다. 첫 체결이 계좌→보유→주문을 잠근 뒤 다음 상대 계좌를 추가 잠그면 서로 다른 종목 worker 사이의 전역 잠금 순서가 깨지고, 예산 잠금과 장마감 fence 보유시간도 늘어날 수 있기 때문입니다. 도입하려면 미리 고정한 비중복 후보 집합의 전체 계좌·보유·주문·예산 키를 전역 순서로 잠그는 별도 알고리즘과 1건/2건 동일 seed MySQL A/B가 필요하며, commit 감소보다 deadlock·account lock p99·close drain이 악화되면 1건 commit을 유지합니다.
- `stock.batch.order-book-execution.worker.*`: Redis ready-symbol 큐를 상시 소비하는 체결 worker 설정입니다. 기본 worker 수는 2, 허용 범위는 1~8이며 빈 큐 대기는 100ms입니다. DB/Redis reconcile은 같은 lifecycle 안의 전용 단일 maintenance thread에서 실행하므로 느린 복구 한 회차가 설정된 체결 worker 슬롯을 점유하지 않습니다. 체결 성공 청크 뒤에는 기본 5ms(`match-yield-ms`) 동안 양보해 LP 공급과 자동 주문 만료가 같은 종목 lock을 얻을 기회를 보장합니다. worker 수는 종목 수·DB pool·실측 TPS보다 먼저 늘리지 않습니다.
- 자동 참여자 주문·만료·발행 인수기관 공급은 한 실행에서 시뮬레이션 clock을 중복 조회하지 않습니다. 종목별 주문 만료와 발행 인수기관 주문은 이미 잠근 session fence가 반환한 `businessEffectiveAt`을 재사용하고, 자동 참여자 profile/symbol shard도 상위 run 시각을 전달받으므로 종목 수에 비례한 clock singleton 조회가 생기지 않습니다.
...
- `stock.batch.order-book-execution.fixed-delay-ms`: Redis 전체 장애나 경량 reconcile 실패 중 누락된 체결 가능 종목을 `stock_order`에서 복구하는 최종 저주기 fallback이며 기본값은 30000ms, 허용 범위는 10000~300000ms입니다. 정상 체결 지연은 신규 주문 after-commit Redis enqueue와 상시 worker가 담당하므로 이 무거운 쿼리 주기를 낮춰 5초 polling으로 되돌리지 않습니다.
...
- 한 symbol 청크는 `DRAINED`, `SESSION_CLOSED`, `MATCH_LIMIT`, `TIME_LIMIT`, `GLOBAL_LIMIT`, `STALE_LIMIT` 종료 사유를 구분합니다. 후보 없음·장 종료는 재큐잉하지 않고, 건수·시간·전체 상한 또는 bounded stale 재시도 상한에 도달한 경우만 추가 `hasExecutablePair` SQL 없이 즉시 재큐잉합니다. 잔액·보유 불일치로 한 주문을 거부한 경우에는 같은 종목 lock 안에서 다음 후보를 계속 찾아 정상 상대 주문이 30초 fallback까지 밀리지 않게 합니다.
- 체결 계측은 후보 조회, transaction 시작 overhead, callback 본문, transaction 종료 overhead, transaction 전체, 계좌·보유·주문 잠금, 원장 쓰기, after-commit callback, symbol 청크를 분리합니다. 종료 overhead에는 JDBC COMMIT/ROLLBACK 외 Spring transaction 종료와 after-commit 시간이 포함되므로 순수 COMMIT으로 단정하지 않고, 별도 after-commit timer와 Performance Schema COMMIT digest를 함께 비교합니다. metric tag에는 symbol·account·order ID를 사용하지 않습니다.
...
- 파일 로그의 공통 루트는 `STOCK_LOG_ROOT`, 배치 전용 경로는 `STOCK_BATCH_LOG_DIR`, 실행 인스턴스 표시는 `STOCK_INSTANCE_ID`로 지정합니다. 파일 로그에는 PID·포트·인스턴스가 포함되고 테스트 프로필은 파일 로그를 기록하지 않습니다. 종목 lock 경합은 매회 WARN 대신 `stock.liquidity.provider.market.symbol.lock.skips`, `stock.auto.market.order.expiry.symbol.lock.skips` 카운터로 확인합니다.
- 자동 참여자 주문의 계획 대비 저장 탈락은 `/actuator/metrics`의 `stock.auto.market.order.decisions`, `stock.auto.market.order.planned`, `stock.auto.market.order.stored`, `stock.auto.market.order.dropped`로 확인합니다. `dropped`의 `reason`은 `side_not_selected`, `open_quantity_limit`, `quantity_multiplier_zero`, `invalid_price`, `insufficient_cash`, `insufficient_holding`, `profile_risk_limit`, `funding_budget_empty`, `funding_budget_reservation_failed`, `session_closed`, `buy_reservation_failed`, `sell_reservation_failed` 중 하나입니다. 전체 저카디널리티 지표에는 계좌·종목을 태그로 넣지 않으며, V2 진단은 `stock.auto.market.profile.decisions(profile,model,action,reason)`와 `stock.auto.market.profile.order.dropped(profile,reason)`로 제한합니다. DB 저장 예외는 기존 `stock.auto.market.order.insert.failures`에서 별도로 확인합니다.
- 자동 참여자 V2는 `ProfileDecision(BUY/SELL/HOLD)` → 실행 가능성 → 프로필 위험 제한 → 실행 스타일 → 주문 계획 순서를 사용합니다. 무보유를 `BUY`로, 현금 부족을 `SELL`로 강제하지 않으며, 선택된 의도가 실행 불가능하면 주문 계획 계층에서 `insufficient_holding` 또는 `insufficient_cash`로 기록하고 반대 주문으로 뒤집지 않습니다. `decisionFrequencyMultiplier`와 `ordersPerDecisionMultiplier` 중 하나가 0이면 의사결정 중지로 판정해 기존 schedule row도 bounded delete하고 claim·계좌상태 bulk 조회를 반복하지 않습니다. 시장조성형은 전체 유동자산의 주식 목표 50%를 해당 계좌에 활성화된 자동장 종목 수로 균등 분할한 종목별 목표와 ±20% 밴드를 사용합니다. 현재 보유 종목 수가 아니라 이미 bulk 조회된 거래 가능 종목 집합을 사용하므로 신규 종목 진입 순서에 따라 목표 합계가 50%를 넘지 않습니다. `pricing_mode=MARKET_MAKING`과 `inventory_mode=TARGET_ALLOCATION`이 모두 설정된 경우에만 밴드 안에서 짝수 개의 수동 매수·매도 호가를 쌍으로 만들고, 합법적인 일일 가격 제한 안에서 상대 최우선 호가를 넘지 못하면 그 호가는 생략합니다. 밴드 밖에서는 재고 정상화 방향만 유지합니다. V2 시장조성 미체결 방향 상한은 종목 1회 주문 상한의 2배이며, 일반 자동 참여자의 전역 미체결 상한 10배를 그대로 사용하지 않습니다.
- 자동 참여자의 `behavior_seed`는 생성 시 UTF-8 `user_key`의 SHA-256 앞 60비트를 양수 정수로 변환해 한 번 저장하고 이후 재계산하지 않습니다. Java 생성·NULL fallback과 MySQL ALTER backfill이 같은 공식을 사용하므로 계정 생성 경로에 따라 시드가 달라지지 않습니다. 기존 운영 151계정의 저장값도 이 공식과 전부 일치하며 저장된 값 자체는 변경하지 않았습니다.
- 기존 V2 시장조성 호가는 최소 30시뮬레이션 초를 유지하고 동일 방향 최우선 호가에서 기본 2틱보다 멀어진 경우에만 조기 취소 대상으로 삼으며, TTL 만료를 포함한 취소·재등록은 방향별 실행당 10건으로 제한합니다. 이 유지보수 조회는 종목당 bounded 후보 조회이며 주문 생성·체결 트랜잭션에는 들어가지 않습니다. 만료 worker가 체결 종목 락을 보유한 동안에는 V2 시장조성 후보를 먼저 조회하고, 양방향 후보가 모두 없으면 호가창 집계를 생략합니다.
- V2의 시간·포트폴리오 프로필은 완료된 일별 snapshot, 종목별 한 번 읽은 호가 상태, due 계좌 cohort를 한 번에 읽은 portfolio/funding/position state만 사용합니다. 주문마다 다일 가격·포트폴리오·배당 원장을 재조회하지 않습니다. 현재 run의 후보가 전부 `V1`이면 5분 신호·45일 이력·전체 포트폴리오·funding/position/performance state와 소액분산형 포트폴리오 노출을 조회하지 않습니다. V2 후보가 있는 종목과 계좌에만 전체 V2 bulk 조회를 적용합니다. 다일 신호는 자동장 실행 1회당 V2 대상 종목만 45일 범위의 단일 bulk query로 읽고 `(symbol, simulation_trade_date, close_run_id)` 인덱스를 사용하므로 계좌 수에 비례하지 않습니다. 소액분산형의 종목 선택도 V2 계좌에만 적용합니다. 주문 수와 실행 주기는 늘리지 않으며 추가 조회는 `(account_id, symbol)` 범위와 기존 주문 인덱스로 제한합니다.
- V2 종목 선택 단계에서 보고서 신호는 방향과 활동 강도를 분리합니다. `NEWS_REACTIVE`의 호재·악재는 절댓값이 같으면 같은 종목 선택 강도를 가지며, 매수·매도 방향은 이후 profile decision의 부호가 결정합니다. V1은 기존 signed score를 유지합니다.
- 위 정책값은 **V2 초기 운영값**이며 실증적으로 확정된 최적값이 아닙니다. 행동 모델은 `stock_auto_participant_profile_config.behavior_model_version`에서 프로필 단위로만 선택하며 같은 프로필의 참여자 일부만 V1/V2로 나누지 않습니다. 전환 후 5~10거래일 동안 프로필별 주문/계좌/일, 체결률, 보유기간, 회전율, 가격 충격, 현금·재고 밴드 이탈시간과 주문·체결 p95/p99를 관찰합니다. 성공 주문 처리량이 기존 기준의 95% 미만이거나 DB commit·row-lock·timeout이 악화되면 해당 프로필 전체를 V1으로 되돌립니다.
- 주기 입금은 자금 공급 정책일 뿐 공통 매수 신호가 아닙니다. 입금 금액이 설정되어 있다는 사실만으로 주문마다 매수 확률을 올리지 않으며, 월급매수형의 매수 행동은 해당 프로필의 방향 정책에서만 결정합니다.
- PAYDAY·DIVIDEND 목적성 예산은 주문 승인 때 예약된 매수 원금까지만 사용합니다. 향후 수수료가 0보다 커지거나 체결가 차이로 실제 비용이 예약액을 넘으면 초과분은 체결 계층이 검증한 일반 계좌 현금이 부담하며, 목적성 예산을 사후 증액하지 않습니다.
...
- 내부 주문장 모드는 자전거래 방지를 위해 같은 계좌의 매수/매도 주문은 매칭하지 않습니다.
