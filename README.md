...
- 축소시장 역할 재구성 ALTER의 기준 파일과 적용 순서는 stock-back README를 따릅니다. batch에 둔 일곱 ALTER 사본은 배포 전에 canonical 사본과 byte 단위로 같아야 하며, 일곱 스키마의 readiness가 모두 통과하기 전에는 기관 shadow, LP, 인수기관 공급 job을 켜지 않습니다.
...
- 액면분할 확정 트랜잭션은 보유·발행량과 가격뿐 아니라 자동시장 기준거래량, 기관 종목 mandate, LP 기준거래량·목표재고·주문상한, 전환 전 listing-auto 수량 기준도 같은 배수로 보정합니다. 그렇지 않으면 분할로 정상 증가한 LP 재고를 과잉 재고로 오판할 수 있습니다. 무상증자·주식배당의 계좌별 정수 배정 후 남는 단주는 유일한 `SYSTEM_CUSTODY` 계정에 비거래 수량으로 입고하고 `stock_security_allocation_ledger`에 남겨, 공시 발행량과 실제 보유량이 보존되도록 합니다.
