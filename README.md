...
- `prod`의 `STOCK_DB_URL`, `STOCK_BATCH_DB_URL`은 query string 없는 기본 JDBC URL로 넣습니다. 공통 JDBC 옵션은 설정 파일에서 `connectTimeout=5000`, `socketTimeout=60000`, `tcpKeepAlive=true`를 기본으로 붙입니다. 배치 업무 statement timeout 기본 30초보다 socket read timeout을 길게 유지하여 쿼리 취소·rollback 전에 연결이 먼저 폐기되지 않게 합니다.
...
- V3 일반 관심 이벤트의 기본 이론상 동시 계획 상한은 `min(profile-worker-count, thread-pool.max-size) × generation-participant-chunk-size × 참여자당 최대 주문 1개`, 즉 현재 기본값으로 225건입니다. 전 종목 강제청산은 별도 청산 계획의 bounded 자식 주문으로 처리해 이 상한에 억지로 섞지 않습니다. 청크를 25보다 키우거나 worker/slot을 늘릴 때는 두 값을 동시에 올리지 않고, 동일 MySQL 데이터·동시성에서 주문 API p95, 체결 p95/p99, commit, row-lock wait, deadlock을 비교합니다.
...
- `stock.batch.post-close.report-aggregation.position-state-account-chunk-size`: 자동 참여자 포지션 상태 재구축이 한 번의 Spring Batch 체크포인트에서 처리하는 동결 계좌 cohort 상한입니다. 기본값은 50, 허용 범위는 1~100입니다. `(close_cycle_id, account_id)` 키셋으로 계좌를 읽고, 체결 경로 계산은 해당 계좌의 `stock_execution` 범위만 조회한 뒤 재진입 포지션 PK만 갱신합니다. 재시작은 마지막 완료 계좌 다음부터 이어지며 같은 cohort 재실행도 멱등입니다.
...
- 자동 참여자 주문의 계획 대비 저장 탈락은 `/actuator/metrics`의 `stock.auto.market.order.decisions`, `stock.auto.market.order.planned`, `stock.auto.market.order.stored`, `stock.auto.market.order.dropped`로 확인합니다. 전체 저카디널리티 지표에는 계좌·종목을 태그로 넣지 않으며 V3 결정과 주문 탈락은 프로필·모델·행동·사유까지만 태그로 사용합니다. DB 저장 예외는 `stock.auto.market.order.insert.failures`에서 별도로 확인합니다.
- 자동 참여자 V3는 거래일별 `OFFLINE/LOW/NORMAL/HIGH` 잠재 상태, 누적위험률 기반 자발적 관심 시계, 모든 보유 종목을 확인하는 강제 가드 시계를 분리합니다. 자발적 관심은 확률적으로 한 종목과 최대 한 주문만 선택하고, 강제 위험축소·마감청산은 관심 확률을 우회하되 현금·보유·가격·호가·세션 제한은 우회하지 않습니다.
...
- V3 수량은 현금·보유·미체결·자산배분·목적성 예산·공격 주문의 반대 호가 깊이·ADV·비상 일일 회전율 중 최솟값을 `safeMaxQuantity`로 먼저 확정한 뒤 소액 편향 분포로 추첨합니다. 추첨 뒤 배율을 다시 곱해 상한에 몰리게 하지 않습니다.
- V3의 다일 가격·포트폴리오·funding/position state는 due 계좌와 대상 종목을 bulk 조회하며 주문마다 다시 읽지 않습니다. 보고서의 절댓값은 관심 강도, 부호는 profile decision 방향에 반영합니다.
- V3 초기 정책값은 실증적으로 확정된 최적값이 아닙니다. 5~10거래일 동안 프로필별 관심/주문/계좌/일, 조건부 실행률, 체결률, 피로도, 회전율, 가격 충격, 강제청산 미완료와 주문·체결 p95/p99를 관찰합니다. 이상 시에는 과거 모델로 되돌리지 않고 정책 리비전을 다음 개장에 교체하거나 감사 사유를 남겨 V3 런타임을 즉시 중지합니다.
