...
class StockBatchJobControllerTest {
    ...
    void getRuntimeControls_getRequest_returnsAllBatchJobRuntimeStatuses() throws Exception {
        mockMvc.perform(get("/internal/stock-batch/v1/jobs/runtime-controls"))
                ...
                .andExpect(jsonPath("$.data[9].jobName").value("liquidity-provider-market"))
                .andExpect(jsonPath("$.data[10].jobName").value("issue-underwriter-market"))
                .andExpect(jsonPath("$.data[11].jobName").value("institution-market"))
                .andExpect(jsonPath("$.data[14].jobName").value("portfolio-settlement"))
                .andExpect(jsonPath("$.data[15].jobName").value("post-close-report-aggregation"))
                .andExpect(jsonPath("$.data[16].jobName").value("market-open-readiness"))
                .andExpect(jsonPath("$.data[17].jobName").value("holding-cleanup"))
                .andExpect(jsonPath("$.data[18].jobName").value("batch-metadata-retention"));
    }
