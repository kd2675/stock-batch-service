...
SET @stock_order_auto_behavior_model_check_exists := EXISTS (
    SELECT 1 FROM information_schema.table_constraints
     WHERE constraint_schema = DATABASE()
       AND table_name = 'stock_order'
...
SET @stock_order_policy_alter_clauses := '';
SET @stock_order_policy_alter_clauses := IF(
    ...
    @stock_order_policy_alter_clauses
);
SET @stock_order_policy_alter_clauses := IF(
    @stock_order_auto_behavior_model_check_exists,
    CONCAT_WS(', ', NULLIF(@stock_order_policy_alter_clauses, ''),
...
SET @stock_order_policy_schema_sql := IF(
    @stock_order_policy_alter_clauses = '',
    'SELECT 1',
    CONCAT('ALTER TABLE stock_order ', @stock_order_policy_alter_clauses)
);
PREPARE stock_order_policy_schema_stmt FROM @stock_order_policy_schema_sql;
EXECUTE stock_order_policy_schema_stmt;
DEALLOCATE PREPARE stock_order_policy_schema_stmt;

ALTER TABLE stock_order
  ADD CONSTRAINT chk_stock_order_auto_behavior_model
  CHECK (auto_behavior_model_version IS NULL OR auto_behavior_model_version = 'V3');

-- The former general-participant market-making path is retired. Remove its
-- write-amplifying hot-ledger index if it exists.
